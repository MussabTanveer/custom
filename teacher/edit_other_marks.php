...
<?php
    ...
    $PAGE->set_url($CFG->wwwroot.'/local/ned_obe/teacher/edit_other_marks.php');
...
<?php
    ...
    if(!empty($_GET['edit']) && !empty($_GET['userid']) && !empty($_GET['otherid']))
    {
        $id=$_GET['edit'];
        $userid=$_GET['userid'];
        $other_id=$_GET['otherid'];
        
        $recofmaxmark=$DB->get_records_sql('SELECT maxmark FROM mdl_manual_other WHERE id = ?',array($other_id));
        ...
        if($recofmaxmark){
            ...
            if(isset($_POST['save']))
            {
                ...
                if($newmarks > $maxmark){
                    ...
                    $sql_update="UPDATE mdl_manual_other_attempt SET obtmark=? WHERE id=?";
                    //echo $id;
                    $DB->execute($sql_update, array($newmarks, $id));
                    $msg = "<font color='green'><b>Marks successfully updated!</b></font><br />";
                    $redirect = "view_other2.php?otherid=$other_id";
                    redirect($redirect);
                }
            }
            if(isset($msg)){
...
?>
    <h3>Edit Other Marks For <?php echo $userid; ?></h3>
...
<?php
    if(isset($_GET['edit'])){
       $id=$_GET['edit'];
$rec=$DB->get_records_sql('SELECT obtmark FROM mdl_manual_other_attempt WHERE id=?',array($id));
...
<?php
    echo "<a href='view_other2.php?otherid=$other_id' > Back </a>";
}
    echo $OUTPUT->footer();
?>
