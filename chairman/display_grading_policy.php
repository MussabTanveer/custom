<?php
    ...
    $PAGE->set_url($CFG->wwwroot.'/local/ned_obe/chairman/display_grading_policy.php');
   ...
   // if(isset($_GET['course'])){
        ...
        $rec=$DB->get_records_sql('SELECT id, name, percentage FROM mdl_grading_policy WHERE name="mid term" OR name="final exam" AND
        ...
        if($rec){
            ...
            if($serial){
                if($sum != 80)
                    echo "<h5 style='color:red'>Grading Policy is not 80%<h5><a href=grading_policy.php?course=$course_id>Add a grading policy item</a>.<br /><br />";
